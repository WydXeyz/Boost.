...
def get_fingerprint(thread):
    try:
        fingerprint = httpx.get(f"https://discord.com/api/v10/experiments", proxies =  {'http://': f'http://{random.choice(open("input/proxies.txt", "r").read().splitlines())}', 'https://': f'http://{random.choice(open("assets/proxies.txt", "r").read().splitlines())}'} if config['proxyless'] != True else None)
                ...
                if join_outcome:
                    ...
                    if config["change_server_nick"]:
                        ...
                        if changed:
                            ...
                            sprint(f"Failed to rename user!", False)
                else:
